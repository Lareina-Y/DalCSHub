...
const User = require("../models/user");
const Course = require("../models/course");
>>>>>>> b82a18b185cdcbe01e8031cbfd3579233736d467
...
router.put('/follow', async (req, res) => {
    ...
    try {
        ...
        if (user.followedCourses.includes(courseId)) {
          return res.status(409).json({ message: 'User is already following the course' });
        }
...
router.put('/unfollow', async (req, res) => {
    ...
    try {
        ...
        // Check if the user is already following the course
        if (!user.followedCourses.includes(courseId)) {
          return res.status(409).json({ message: 'User is not following the course' });
        }
